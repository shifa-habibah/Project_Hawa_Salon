...
{
    public partial class UbahPerawatanRambut : Form
    {
        public UbahPerawatanRambut()
        {
            InitializeComponent();
        }
        ...
        private void btnCari_Click(object sender, EventArgs e)
        {
            try
            {
                if (!string.IsNullOrEmpty(txtID.Text.Trim()))
                {
                    ...
                    {
                        using (SqlCommand sqlCmd = new SqlCommand("sp_CariPrRambut", sqlConn))
                        {
                            sqlConn.Open();
                            sqlCmd.CommandType = System.Data.CommandType.StoredProcedure;
                            sqlCmd.Parameters.AddWithValue("Id_prwt_rambut", txtID.Text.Trim());
